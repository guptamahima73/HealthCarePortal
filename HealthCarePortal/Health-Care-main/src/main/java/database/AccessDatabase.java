...
public class AccessDatabase {
    ...
    private static final String DB_USER = "swasthsaarthi";
    ...
    public boolean insertNotification(Notification notification) {
        String idSql = "SELECT MAX(notificationId) FROM Notification";
        String sql = "INSERT INTO Notification (notificationId, userId, message, isRead) VALUES (?, ?, ?, ?)";
        int nextId = 1;
    ...
    public List<Notification> getNotificationsByUserId(int userId) {
        ...
        String sql = "SELECT notificationId, userId, message, isRead, createdAt FROM Notification WHERE userId = ? ORDER BY createdAt DESC";
    ...
    public boolean insertAppointment(Appointment appointment) {
        String idSql = "SELECT MAX(appointmentId) FROM Appointment";
        String sql = "INSERT INTO Appointment (appointmentId, healthId, userId, appointmentDate, status) VALUES (?, ?, ?, ?, ?)";
        int nextId = 1;
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            int rows = pstmt.executeUpdate();
            if (rows > 0) {
                // Create notification for the healthworker
                Notification notification = new Notification();
                notification.setUserId(appointment.getHealthId());
                notification.setMessage("New appointment booked for " + appointment.getAppointmentDate() + ".");
                notification.setRead(false);

                insertNotification(notification); // Insert notification into DB
